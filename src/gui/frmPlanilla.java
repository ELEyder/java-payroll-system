...
public class frmPlanilla extends JFrame implements Runnable {
	...
	JLabel lblSubMenu01,lblSubMenu02,lblSubMenu03,lblSubMenu04,lblSubMenu05,lblSubMenu06,lblSubMenu07,lblSubMenu08,lblSubMenu09,lblSubMenu10,lblSubMenu11,lblSubMenu12;	
	...
	String[][][] aSubMenu = {
		{
			...
			{ "Iniciar período","Declarantes","Sede","Centro de costos","Departamento","Moneda","Conceptos","Comisión AFPs","Régimen pensionario","Turno","Montos por hora","Bancos" }
		},
		{
	...
	public frmPlanilla() {
		...
		pnlMenu = new JPanel();
		pnlMenu.setBounds(1,50,300,650);
	...
	protected void form_windowsOpened() {
		...
		aLblSubMenu = new JLabel[] { lblSubMenu01,lblSubMenu02,lblSubMenu03,lblSubMenu04,lblSubMenu05,lblSubMenu06,lblSubMenu07,lblSubMenu08,lblSubMenu09,lblSubMenu10,lblSubMenu11,lblSubMenu12 };
	...
	protected void imgMenu_mouseClicked(int menu) {
		...
		for( int i=0; i < aMenu[MenuPlanilla].length; i++ ) {
			aLblMenu[i].setBounds(10, 80 + i*30 + ( i <= Menu ? 0 : altura ) ,200,25);
			...
			aImgMenu[i].setIcon(new ImageIcon(getClass().getResource("/gui/img/" + ( i==0  ? "arriba" : "abajo" ) + ".png")));
